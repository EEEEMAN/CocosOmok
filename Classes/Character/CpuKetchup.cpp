#include "CpuKetchup.h"

CpuKetchup::CpuKetchup(){
	characterID = CharacterID::CHAR_KETCHUP;
	readTurn = 1;
	imageName = "ketchup.png";
	name = "케찹";
	stone = new Stone_Ketchup();
	money = 75;
	stoneProbability = 0.8;
}

int CpuKetchup::seachString(char* str) const {
	int util = 0;
...
int CpuKetchup::utility(GNode* node) const{
	if (node->isTerminal()){ //종료상태일경우
		if (node->getTurn() % 2 == 1){
			return -INF;
		}
		else{
